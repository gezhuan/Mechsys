...
/*  Reddy's book: Example 5.4.1 (pag. 278)
...
#include "fem/elems/elasticbeam.h"
...
int main(int argc, char **argv) try
{
	...
	g.SetNNodes (3);
	g.SetNode   (0,   0.0,   0.0);
	g.SetNode   (1,   0.0, 144.0);
	g.SetNode   (2, 144.0, 252.0);

	// Elements
	g.SetNElems (2);
	g.SetElem   (0, "ElasticBeam");
	g.SetElem   (1, "ElasticBeam");
	...
	g.Nod(0)->Bry("ux", 0.0)->Bry("uy",  0.0)->Bry("rz", 0.0);
	g.Nod(2)->Bry("ux", 0.0)->Bry("uy",  0.0)->Bry("rz", 0.0);
	g.Nod(1)->Bry("fy",-1.0);

	// Check stiffness matrices
	double err_ke = 0.0;
	LinAlg::Matrix<double> Ke0,  Ke1;
	LinAlg::Matrix<double> Ke0c, Ke1c; // correct matrices
	Ke0c.Resize(6,6);
	Ke1c.Resize(6,6);
