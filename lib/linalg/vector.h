...
public:
	// Constructors
	Vector () : _size(0), _values(NULL), _ns(&Util::_8s) {}  ///< Default constructor
	...
	public:
		...
		{ 
#ifndef DNDEBUG
   			if (_values==NULL) throw new Fatal(_("Vector::CommaAssign::CommaAssign (_values==NULL). The vector must be resized prior to use this method."));
#endif
		...
		CommaAssign & operator, (Value_T const & Num) 
		{
			_index++;
#ifndef DNDEBUG
   			if (_index>=_size) throw new Fatal(_("Vector::CommaAssign::operator, (_index>=_size). There are too many values in the comma expression to be assigned to this vector."));
#endif
			_values[_index] = Num;
			return *this;
		}
...
{
#ifndef DNDEBUG
   	if (Size<=0) throw new Fatal(_("Vector::Vector(Size): (Size<=0). The number of components (Size=%d) must be greater than zero."),Size);
#endif
	_size   = Size;
...
{
#ifndef DNDEBUG
   	if (_values==NULL) throw new Fatal(_("Vector::Size: (_values==NULL). The vector must be resized prior to get the pointer to its values."));
#endif
   	return _values;
}
...
{
#ifndef DNDEBUG
   	if (Size<=0) throw new Fatal(_("Vector::Resize(Size): (Size<=0). The number of components (Size=%d) must be greater than zero."),Size);
#endif
...
{
#ifndef DNDEBUG
   	if (_values==NULL) throw new Fatal(_("Vector::SetValues: (_values==NULL). The vector must be resized prior to set its values."));
#endif
	for (int i=0; i<_size; ++i)
		_values[i] = Value; 
}
...
{
#ifndef DNDEBUG
	if (&R==this) throw new Fatal(_("Vector::operator= The right-hand-size of this operation (LHS = RHS) must not be equal to the LHS."));
#endif
...
{
#ifndef DNDEBUG
	if (_values==NULL  ) throw new Fatal(_("Vector::operator+= (_values==NULL). The vector must be resized prior to use this method."));
	if (R.Size()!=_size) throw new Fatal(_("Vector::operator+= (R.Size()!=_size). The number of components of the LHS (%d) must be equal to the number of components of the RHS (%d)."),R.Size(),_size);
#endif
	// Add values
	for (int i=0; i<_size; ++i)
		_values[i] += R._values[i];
...
#ifndef DNDEBUG
	if (_values==NULL  ) throw new Fatal(_("Vector::operator() (_values==NULL). The vector must be resized prior to use this method."));
	if (i<0 || i>=_size) throw new Fatal(_("Vector::operator() (i<0 || i>=_size). The index (i=%d) for a component must be greater than/or equal to zero and smaller than the number of components (size=%d)."),i,_size);
#endif
	return _values[i];
}
