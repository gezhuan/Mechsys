...
@try_catch
def cb_eatt_setety  (evt,val): di.props_set_fem_all_stg ('eatts', evt-EVT_INC, 1, int(val))
@try_catch
def cb_eatt_setpty  (evt,val): di.props_set_fem_all_stg ('eatts', evt-EVT_INC, 2, int(val))
@try_catch
def cb_eatt_setmat  (evt,val): di.props_set_fem_all_stg ('eatts', evt-EVT_INC, 3, int(val))
@try_catch
        ...
        if len(stages)>0:
            ...
            for tag, k in etags:
                ...
                if num==1:
                    ...
                    Draw.Menu       (d['etymnu'],  EVT_INC+i,   c+ 60, r,     80,   rh, int(v[1])+1,       'Element type: ex.: Quad4, Hex8',      cb_eatt_setety)
                    Draw.Menu       (d['ptymnu'],  EVT_INC+i,   c+140, r,     80,   rh, int(v[2])+1,       'Problem type: ex.: PStrain, Equilib', cb_eatt_setpty)
                    Draw.Menu       (matmnu,       EVT_INC+i,   c+220, r,    100,   rh, int(v[3])+1,       'Choose material',                     cb_eatt_setmat)
