...
public:
    ...
    // Constructor & Destructor
     HMStressUpdate ();
    ~HMStressUpdate () { if (ODE!=NULL) delete ODE;  if (sta_1!=NULL) delete sta_1;  if (sta_ME!=NULL) delete sta_ME; }
...
inline Model::HMStressUpdate::HMStressUpdate ()
    : Mdl      (NULL),
      DbgFun   (NULL),
      DbgDat   (NULL),
      ODE      (NULL),
...
inline void Model::HMStressUpdate::SetModel (Model const * TheMdl)
{
    Mdl = TheMdl;
    ncp = Mdl->NCps;
    niv = Mdl->NIvs;
...
inline void Model::HMStressUpdate::SetScheme (String const & Name)
{
    if      (Name=="ME")       Scheme = ME_t;
    else if (Name=="SingleFE") Scheme = SingleFE_t;
    else if (Name=="RK")       Scheme = RK_t;
    else throw new Fatal("HMStressUpdate::SetScheme: Scheme named %s is invalid",Name.CStr());
}

inline void Model::HMStressUpdate::Update (Vec_t const & DEps, State * Sta, Vec_t & DSig)
{
    ...
    // check
    if (Mdl==NULL) throw new Fatal("Model::HMStressUpdate::Update: SetModel must be called before calling this method");
    ...
    else if (Scheme==ME_t)
    {
        ...
        if (SS>=MaxSS) throw new Fatal("HMStressUpdate::Update: Modified-Euler (local) did not converge after %d substeps",SS);
    }
    else if (Scheme==RK_t)
    {
    ...
    else throw new Fatal("HMStressUpdate::Update: Scheme is not available yet");
...
inline void Model::HMStressUpdate::GetInfo (std::ostream & os, bool Header) const
{
    String buf;
    if (Scheme==ME_t)
    {
        if (Header)
        {
            os << "\n" << TERM_BLACK_WHITE << "----------------------------- HMStressUpdate/Scheme: ME ------------------------------" << TERM_RST << "\n\n";
    ...
    {
        if (Header) os << "\n" << TERM_BLACK_WHITE << "----------------------------- HMStressUpdate/Scheme: SingleFE ------------------------" << TERM_RST << "\n\n";
    }
    else if (Scheme==RK_t)
    {
        if (Header)
...
inline int Model::HMStressUpdate::_RK_fun (double t, double const Y[], double dYdt[])
{
...
inline void Model::HMStressUpdate::_RK_up_fun (double t, double Y[])
{
    // current strain, stress, and internal values
    sta->Eps = eps0 + t*deps;
    for (size_t i=0; i<ncp; ++i) sta->Sig(i) = Y[    i];
