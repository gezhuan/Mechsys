/* plot-xy - Copyright (C) 2007 Dorival de Moraes Pedroso */
...
inline PlotXY::CurveProps & PlotXY::SetCurve(size_t i)
{
	// Check
	if (i<0 || i>=_X.Size())
...
inline void PlotXY::draw()
{
	...
	if (_X.Size()>=1 && _Y.Size()>=1)
	{
		...
		while (k<_X.Size())
		{
			...
			// Draw text
			char buf[256];
			snprintf (buf, 250, "y=%g", (*_Y[k])[0]);
			fl_color (FL_BLACK);
			fl_font  (0, 10);
			fl_draw  (buf, _x((*_X[k])[0]), _y((*_Y[k])[0]), 0,0, FL_ALIGN_LEFT);
