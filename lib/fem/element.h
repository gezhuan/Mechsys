...
public:
	...
	bool         HasKey    (          Str_t Key) const                  { CHECKGEPE("HasKey"   ) return _pe->HasKey(Key);                     } ///< Has key for computed values at the CG of the element. Ex.: Key="Sx", "Sxy", "Ex", etc.
	void         GetKeys   (Array<String> & Keys) const                 { CHECKGEPE("ValKeys"  )        _pe->GetKeys(Keys);                   } ///< Keys for computed values at the CG of the element. Ex.: Key="Sx", "Sxy", "Ex", etc.
