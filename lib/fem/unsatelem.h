...
#ifndef MECHSYS_FEM_UNSATELEM_H
#define MECHSYS_FEM_UNSATELEM_H
...
class UnsatElem : public virtual Element
{
public:
	// Constants
	...
	// Constructor
	UnsatElem () : _gam(0.0), _d(-1), _nd(-1), _nl(-1), _gw(-1) {}
	...
	size_t nOrder1Matrices () const { return 4; }                                                                                                              ///< Number of first order matrices: K:Stiffness, L1:CouplingMatrix1, L2:CouplingMatrix2 and M:MassMatrix.
...
protected:
	...
	Array<double>          _v;      ///< Specific volume at each integration point
...
private:
	...
	double _Sr(double pwp) const;
	double _phi(double pwp) const; 
	double _dSr_dpwp(double pwp) const;
	void   Ke(LinAlg::Matrix<double> & Ke) const;


}; // class UnsatElem

// NDE[_ndim-1]                   1D  2D  3D
const size_t UnsatElem::NDE[3] = { 1,  2,  3};

// UD[_ndim-1][iDOF]                  1D                   2D                    3D
const size_t UnsatElem::ND[3]       = { 2,                   3,                    4};
const char   UnsatElem::UD[3][4][4] = {{"ux","pwp","",""},  {"ux","uy","pwp",""},  {"ux","uy","uz","pwp"}};
const char   UnsatElem::FD[3][4][4] = {{"fx","vol","",""},  {"fx","fy","vol",""},  {"fx","fy","fz","vol"}};

// LB[_geom-1][iLabel]
const size_t UnsatElem::NL[3]        = { 3, 19, 22 };
const char   UnsatElem::LB[3][22][4] = {
	{"Ea", "Sa", "Vx",  ""   , ""   , ""   , ""  , ""   , ""  , ""   , ""   , ""   , ""  , ""  , ""  , ""  , ""  , ""  , ""  , ""  , ""  , "" }, // 1D
...
inline bool UnsatElem::CheckModel() const
{
	if (_gw<0.0 || _De.Rows()==0 || _Ke.Rows()==0) return false;
	return true;
}

inline bool UnsatElem::IsEssential(char const * Name) const
{
	for (int i=0; i<_nd; ++i) if (strcmp(Name,UD[_d][i])==0) return true;
	return false;
}

inline void UnsatElem::SetModel(char const * ModelName, char const * Prms, char const * Inis)
{
	// Check _ndim
	if (_ndim<1)               throw new Fatal("UnsatElem::SetModel: The space dimension (SetDim) must be set before calling this method");
	if (CheckConnect()==false) throw new Fatal("UnsatElem::SetModel: Connectivity is not correct. Connectivity MUST be set before calling this method");

	/* "gw=10 E=200 nu=0.2 k=1.0e-5 kaps=0.002" */
	LineParser lp(Prms);
	Array<String> names;
	Array<double> values;
	...
	for (size_t i=0; i<names.Size(); ++i)
	{
		...
		else throw new Fatal("UnsatElem::SetModel: Parameter name (%s) is invalid",names[i].CStr());
	}

	// Check
	if (_gw<0.0)            throw new Fatal("UnsatElem::SetModel: GammaW must be provided (and positive). gw==%f is invalid",_gw);
	if (E<=0.0)             throw new Fatal("UnsatElem::SetModel: Young modulus (E) must be provided (and positive). E==%f is invalid",E);
	if (nu<0.0 || nu>0.499) throw new Fatal("UnsatElem::SetModel: Poisson ratio (nu) must be provided (and in the range: 0 <= nu < 0.5). nu==%f is invalid",nu);
	if (k<0.0)              throw new Fatal("UnsatElem::SetModel: Isotropic permeability must be provided (and positive). k=%f is invalid",k);
...
inline void UnsatElem::SetProps(char const * Properties)
{
	/* "gam=20.0 */
...
inline Element * UnsatElem::Connect(int iNodeLocal, FEM::Node * ptNode)
{
	// Check
	if (_n_nodes<1)               throw new Fatal("UnsatElem::Connect: __Internal Error__: There is a problem with the number of nodes: maybe derived elemet did not set _n_nodes");
	if (_connects.Size()<1)       throw new Fatal("UnsatElem::Connect: __Internal Error__: There is a problem with connectivity array: maybe derived elemet did not allocate _connect");
	if (_ndim<0 || _d<0 || _nd<0) throw new Fatal("UnsatElem::Connect: __Internal Error__: There is a problem with _ndim=%d, _d=%d, or _nd=%d\n (_ndim=space dimension, _d=dimension index==_ndim-1, and _nd=number of degrees of freedom)",_ndim,_d,_nd);
...
inline void UnsatElem::UpdateState(double TimeInc, LinAlg::Vector<double> const & dUglobal, LinAlg::Vector<double> & dFint)
{
	...
	LinAlg::Matrix<double> Me;                   // mass matrix
	...
	df   = Ke*du + Ce*dp;
	dvol = Le*du + Me*dp + TimeInc*He*p + TimeInc*Qb;
...
inline void UnsatElem::ApplyBodyForces()
{
...
inline void UnsatElem::GetLabels(Array<String> & Labels) const
{
...
inline void UnsatElem::CalcDepVars() const
{
	if (_is_active==false) throw new Fatal("UnsatElem::CalcDepVars: This element is inactive (ID=%d, Tag=%d)",_my_id,_tag);
}

inline double UnsatElem::Val(int iNodeLocal, char const * Name) const
{
	// Displacements
...
inline double UnsatElem::Val(char const * Name) const
{
	throw new Fatal("UnsatElem::Val: Feature not implemented yet");
}

inline void UnsatElem::ClearDispAndStrains()
{
	if (_is_active==false) return;
...
inline void UnsatElem::SetActive(bool Activate)
{
	...
	else throw new Fatal("UnsatElem::SetActive: Deactivation (excavation) is not available yet");
}

inline void UnsatElem::OutInfo(std::ostream & os) const
{
	for (size_t i=0; i<_n_int_pts; i++)
	{
...
inline void UnsatElem::Order0MatMap(size_t Index, Array<size_t> & RowsMap, Array<size_t> & ColsMap, Array<bool> & RowsEssenPresc, Array<bool> & ColsEssenPresc) const
{
...
inline void UnsatElem::Order0VecMap(size_t Index, Array<size_t> & RowsMap) const
{
	Array<bool> rows_essen_presc;
	_compute_flow_map (RowsMap, rows_essen_presc);
}

inline void UnsatElem::Order0Matrix(size_t index, LinAlg::Matrix<double> & M) const
{
	_compute_H(M);
}

inline void UnsatElem::Order0Vector(size_t Index, LinAlg::Vector<double> & V) const
{
	if (Index==0)
	{
		V.Resize(_n_nodes);
...
inline void UnsatElem::Order1MatMap(size_t Index, Array<size_t> & RowsMap, Array<size_t> & ColsMap, Array<bool> & RowsEssenPresc, Array<bool> & ColsEssenPresc) const
{
	if (Index==0)
	{
...
inline void UnsatElem::Order1Matrix(size_t Index, LinAlg::Matrix<double> & M) const
{
...
inline void UnsatElem::B_Matrix(LinAlg::Matrix<double> const & derivs, LinAlg::Matrix<double> const & J, LinAlg::Matrix<double> & B) const
{
	/* OBS.:
	...
	switch (_geom())
	{
		...
		default: throw new Fatal("UnsatElem::B_Matrix: B_Matrix() method is not available for GeometryType==%d",_geom());
	}
}

inline void UnsatElem::Bp_Matrix(LinAlg::Matrix<double> const & derivs, LinAlg::Matrix<double> const & J, LinAlg::Matrix<double> & Bp) const
{
	Bp = inv(J)*derivs;
}
...
inline void UnsatElem::_dist_to_face_nodes(char const * Key, double const FaceValue, Array<Node*> const & FaceConnects) const
{
...
inline void UnsatElem::_compute_K(LinAlg::Matrix<double> & Ke) const
{
	/* Stiffness K:
	   ===========
...
inline void UnsatElem::_compute_C(LinAlg::Matrix<double> & Ce) const
{
	/*  Coupling Matrix L1:  ( n_dim*_n_nodes x _n_nodes )
	    ==================================================
	
	                  /    T      T                            /    T        T            
	         [C]   =  | [B]  * {m} * phi(Sr) * {N}  * dV   -   | [B]  * {dep} * {N}  * dV  // coupled unsaturated
	                  /    u                      p            /    u             p      
	...
	*/
	...
	// Allocate pore-presure vector
	LinAlg::Vector<double>  p(_n_nodes); 
	
	// Assemble pore-presure vector
	size_t nde =       NDE[_d]; // nDOFs Equilibrium
	size_t ndf = _nd - NDE[_d]; // nDOFs Flow
	for (size_t i=0; i<_n_nodes; ++i)
	{
	...
	LinAlg::Vector<double> de;     // Elastic stiffness related to suction
	...
	for (size_t i=0; i<_n_int_pts; ++i)
	{
		...
		// Calculate the elastic stiffnes related to suction
		double pwp = (trn(shape)*p)(0);
		if (pwp>0)
		{
			double suc=pwp;
			LinAlg::Vector<double> ce = _ks/(3.0*_v[i]*(suc+_patm))*m;
			de = _De*ce;
		}
		else de = 0.0*m;
		Ce += trn(B)*m*trn(shape)*phi*det(J)*w - trn(B)*de*trn(shape)*det(J)*w;
	}
}

inline void UnsatElem::_compute_L(LinAlg::Matrix<double> & Le) const
{

	/*
	  	 Coupling Matrix L:  ( _n_nodes x n_dim*_n_nodes )
	  	 =================================================
	         
	                   /                T      
	           [L]  = -| {N}  * Sr * {m} * [B]  * dV     // coupled unsaturated
	                   /    p                u  
	...
	*/           
	...
	// Resize L
	L.Resize(_n_nodes, _ndim*_n_nodes); // sum(Bt*m*Np*det(J)*w)
	L.SetValues(0.0);
	...
	for (int i_ip=0; i_ip<_n_int_pts; ++i_ip)
	{
		...
		double pwp = (trn(shape)*p)(0);
		double Sr  = _Sr(pwp); // Degree of saturation

		L += -shape*Sr*trn(m)*B*det(J)*w;
	}
}

inline void UnsatElem::_compute_M(LinAlg::Matrix<double> & M) const 
{
	//	
	...
	for (int i_ip=0; i_ip<_n_int_pts; ++i_ip)
	{
		...
		Derivs(r,s,t, derivs);            // Calculate Derivatives of Shape functions w.r.t local coordinate system
		Jacobian(derivs, J);              // Calculate J (Jacobian) matrix for i_ip Integration Point
		double _dSr_dPp = _calc_dSr_dPp();
		double n = (_v[i]-1.0)/_v[i];

		// Calculate the M matrix
		M += shape*trn(shape)*n*_dSr_dPp*det(J)*w;
	}
} 

inline void UnsatElem::_compute_H(LinAlg::Matrix<double> & He) const
{
	...
	*/
	...
	for (size_t i=0; i<_n_int_pts; ++i)
	{
		...
		double Ke = _Ke(pwp);
		He += -trn(Bp)*Ke*Bp*det(J)*w/_gw;
	}
}

inline void UnsatElem::_compute_Qb(LinAlg::Vector<double> & Qb) const 
{
	//	
	//	 Matrix Qb:
	//	 ============================
	...
	for (size_t i_ip=0; i_ip<_n_int_pts; ++i_ip)
	{
		...
		LinAlg::Matrix<double> Ke; _Ke(pwp, Ke);
		Qb += trn(Bp)*_Ke*b*det(J)*w/_gw;
	}
} 

inline double UnsatElem::_Sr(double pwp) const 
{
	if (pwp >= 0.0) // if saturated
	    return 1.0;
	else
	    return 1.0/pow(1.0+pow(pwp/_a,_b),_c); // Degree of saturation
}

inline double UnsatElem::phi(double pwp) const 
{
	return _Sr(pwp);
} 

inline double UnsatElem::_dSr_dpwp(double pwp) const 
{
	if (pwp >= 0.0) // if saturated
	    return 0.0;
	else
	{
	    double  dSr_dsuc = -_b*_c*pow(pwp/_a,_b-1.0)/(_a*pow(1.0+pow(pwp/_a,_b),_c+1.0));
	    double dsuc_dpwp = -1.0;
	    double  dSr_dpwp = dSr_dsuc*dsuc_dpwp;
	    return dSr_dpwp;
	}
}

inline void UnsatElem::Ke(LinAlg::Matrix<double> & Ke) const 
{

	LinAlg::Matrix<double> I; identity(3, I);
	Ke = (_ks*pow(Sr(),_m))*I;
} 

inline void UnsatElem::_compute_equilib_map(Array<size_t> & RowsMap, Array<bool> & RowsEssenPresc) const
{
...
inline void UnsatElem::_compute_flow_map(Array<size_t> & RowsMap, Array<bool> & RowsEssenPresc) const
{
...
inline double UnsatElem::_val_ip(size_t iIP, char const * Name) const
{
	...
	else throw new Fatal("UnsatElem::_val_ip: Name==%s if not available for this element",Name);
}

}; // namespace FEM

#endif // MECHSYS_FEM_UNSATELEM_H
