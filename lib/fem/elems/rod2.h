...
public:
	// Constructor
	Rod2 () : _gam(0.0), _E(-1), _A(-1) { NNodes=2; Conn.Resize(NNodes); Conn.SetValues(NULL); }
...
inline void Rod2::UpdateState(double TimeInc, Vec_t const & dUglobal, Vec_t & dFint)
{
...
inline void Rod2::Order1Matrix(size_t Index, Mat_t & Ke) const
{
	if (_ndim==2)
	{
		double dx = Conn[1]->X()-Conn[0]->X();
...
inline void Rod2::B_Matrix(Mat_t const & derivs, Mat_t const & J, Mat_t & B) const
{
...
inline void Rod2::_transf_mat(Mat_t & T) const
{
