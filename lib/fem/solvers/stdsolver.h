...
#ifndef MECHSYS_FEM_STDSOLVER_H
#define MECHSYS_FEM_STDSOLVER_H
...
public:
    ...
    // Constructor
    STDSolver (Domain & Dom, SDPair const & Flags, pOutFun OutFun=NULL, void * OutDat=NULL,
                                                   pOutFun DbgFun=NULL, void * DbgDat=NULL); ///< Allocate solver object

    // Methods
    String Name           () const { return "STDSolver"; }
...
inline STDSolver::STDSolver (Domain & Dom, SDPair const & Flags, pOutFun OutFun, void * OutDat, pOutFun DbgFun, void * DbgDat)
    : Solver   (Dom, Flags, OutFun, OutDat, DbgFun, DbgDat),
      Inc      (0),
      Stp      (0),
      It       (0),
      Scheme   (ME_t),
      ...
      RKSTOL   (1.0e-2)
{
    ...
    if (Flags.HasKey("fe"       )) Scheme   = (static_cast<int>(Flags("fe"))>0 ? FE_t : Scheme);
    if (Flags.HasKey("me"       )) Scheme   = (static_cast<int>(Flags("me"))>0 ? ME_t : Scheme);
    if (Flags.HasKey("nr"       )) Scheme   = (static_cast<int>(Flags("nr"))>0 ? NR_t : Scheme);
    if (Flags.HasKey("calcwork" )) CalcWork = (static_cast<int>(Flags("calcwork"))>0);
    if (Flags.HasKey("nss"      )) nSS      = (static_cast<int>(Flags("nss"     ))>0);
    if (Flags.HasKey("ssout"    )) SSOut    = (static_cast<int>(Flags("ssout"   ))>0);
    if (Flags.HasKey("ctetg"    )) CteTg    = (static_cast<int>(Flags("ctetg"   ))>0);
    if (Flags.HasKey("modnr"    )) ModNR    = (static_cast<int>(Flags("modnr"   ))>0);
    if (Flags.HasKey("hm"       )) DampTy   = (static_cast<int>(Flags("hm")      )>0 ? HMCoup_t   : DampTy );
    if (Flags.HasKey("ray"      )) DampTy   = (static_cast<int>(Flags("ray")     )>0 ? Rayleigh_t : DampTy );
    if (Flags.HasKey("rk"       )) DScheme  = (static_cast<int>(Flags("rk")      )>0 ? RK_t       : DScheme);
    if (Flags.HasKey("am"       )) DampAm   = Flags("am");
    if (Flags.HasKey("ak"       )) DampAk   = Flags("ak");
    if (Flags.HasKey("maxit"    )) MaxIt    = static_cast<int>(Flags("maxit"));
    if (Flags.HasKey("tolr"     )) TolR     = Flags("tolr");
    if (Flags.HasKey("rkstol"   )) RKSTOL   = Flags("rkstol");
    if (Flags.HasKey("nldt_nsml")) NLSteps.Set("nsml", Flags("nldt_nsml"));
    if (Flags.HasKey("nldt_nn"  )) NLSteps.Set("nn"  , Flags("nldt_nn"  ));
    if (Flags.HasKey("nldt_n"   )) NLSteps.Set("n"   , Flags("nldt_n"   ));
    if (Flags.HasKey("nldt_ll"  )) NLSteps.Set("ll"  , Flags("nldt_ll"  ));
    if (Flags.HasKey("nldt_sch" )) NLSteps.Set("sch" , Flags("nldt_sch" ));
    if (Flags.HasKey("nldt_m"   )) NLSteps.Set("m"   , Flags("nldt_m"   ));

    //if (rkscheme!="") Sol.RKScheme = rkscheme;
}

inline void STDSolver::Solve (size_t NInc, char const * FileKey)
{
    // info
...
inline void STDSolver::TransSolve (double tf, double dt, double DtOut, char const * FileKey)
{
    // info
    Util::Stopwatch stopwatch(/*activated*/WithInfo);
...
inline void STDSolver::DynSolve (double tf, double dt, double DtOut, char const * FileKey)
{
    // info
    Util::Stopwatch stopwatch(/*activated*/WithInfo);
    ...
    else if (DScheme==RK_t)
    {
        ...
        Numerical::ODESolver<STDSolver> ode(this, &STDSolver::_RK_func, nvars, RKScheme.CStr(), RKSTOL, dt);
...
inline void STDSolver::AssembleKA ()
{
...
inline void STDSolver::AssembleKMA (double C1, double C2)
{
    if (CteTg && A11.Top()>0) return; // constant tangent matrices => linear problems
...
inline void STDSolver::AssembleKCMA (double C1, double C2, double C3)
{
    if (CteTg && A11.Top()>0) return; // constant tangent matrices => linear problems
    K11.ResetTop(); // reset top (position to insert new values) => clear triplet
    K12.ResetTop();
...
inline void STDSolver::Initialize (bool Transient)
{
    // info
    Util::Stopwatch stopwatch(/*activated*/WithInfo);
...
#else
        throw new Fatal("STDSolver::Initialize: Parallel code is not available (not compiled)");
#endif
    }
    else
    {
...
inline void STDSolver::SetIncsW (size_t NInc, bool NonLinWei)
{
    IncsW.Resize (NInc);
    if (NonLinWei)
    {
...
inline bool STDSolver::ResidOK () const
{
...
inline void STDSolver::_aug_and_set_A ()
{
    // augment A11
    if (FEM::Domain::PARA)
    {
...
inline void STDSolver::_wrn_resid ()
{
...
inline void STDSolver::_cal_resid ()
{
    // calculate residual
    R = F - F_int;
...
inline void STDSolver::_cor_resid (Vec_t & dU, Vec_t & dF)
{
    if (!CorR) return;
    ...
    if (it>=MaxIt) throw new Fatal("STDSolver::_cor_resid: Residual correction did not converge after %d iterations.\n\t(%e>=%e)  (NormR>=TolR*MaxNormF).\n\tNormR         = %g\n\tMaxNormF      = %g\n\tTolR          = %e\n\tTolR*MaxNormF = %e",it,NormR,TolR*MaxNormF,NormR,MaxNormF,TolR,TolR*MaxNormF);
    if (it>It) It = it;
}

inline void STDSolver::_FE_update (double tf)
{
...
inline void STDSolver::_ME_update (double tf, char const * FNK)
{
    // auxiliar vectors
    ...
    if (Stp>=MaxSS) throw new Fatal("STDSolver:_ME_update: Modified-Euler (global integration) did not converge for %d steps",Stp);
...
inline void STDSolver::_NR_update (double tf)
{
    It = 0;
    double dt = (tf-Dom.Time)/nSS;
    for (Stp=0; Stp<nSS; ++Stp)
...
inline void STDSolver::_TH_update (double tf, double Dt)
{
    // timestep
...
inline void STDSolver::_GN22_update (double tf, double Dt)
{
    ...
    while (Dom.Time<tf)
    {
        ...
        for (It=0; It<MaxIt; ++It)
        {
            ...
            if (Util::IsNan(normdU)) throw new Fatal("STDSolver::_GN22_update: normdU is NaN");
#endif
            ...
            if (Util::IsNan(normFint)) throw new Fatal("STDSolver::_GN22_update: normFint is NaN");
#endif
...
#ifdef DO_DEBUG
            if (Util::IsNan(NormR)) throw new Fatal("STDSolver::_GN22_update: NormR is NaN");
            printf("NormR = %g\n",NormR);
#endif
            if (ResidOK()) break;
        }
...
inline void STDSolver::_time_print (char const * Comment)
{
    if (Comment!=NULL)
    {
...
inline void STDSolver::_VUIV_to_Y (double Y[])
{
...
inline void STDSolver::_Y_to_VUIV (double const Y[])
{
    for (size_t i=0; i<NEq; ++i)
    {
        V(i) = Y[i];
...
inline int STDSolver::_RK_func (double t, double const Y[], double dYdt[])
{
...
{
    return new STDSolver (Dom, Flags, OutFun, OutDat, DbgFun, DbgDat);
}

int STDSolverRegister()
{
    SolverFactory["STDSolver"] = STDSolverMaker;
    return 0;
}
