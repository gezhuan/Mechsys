...
{
	if (strcmp(DOFName,"T")==0) return true;
...
inline void HeatElem::UpdateState(double TimeInc, LinAlg::Vector<double> const & dUglobal, LinAlg::Vector<double> & dQint)
{
	// Allocate (local/element) displacements vector
	LinAlg::Vector<double> dT(_n_nodes); // Delta disp. of this element
	...
	LinAlg::Vector<double> dQ(_n_nodes); // Delta internal force of this element
	dQ.SetValues(0.0);
	...
	for (size_t i=0; i<_n_nodes; ++i)
	{
		...
		dQint(_connects[i]->DOFVar("F").EqID) += dQ(i);
	}
}
...
inline void HeatElem::GetLabels(Array<String> & Labels) const
{
	...
	Labels.Resize(2);
	Labels[0]="T"; // Temperature
	Labels[1]="F"; // Heat flow
}
...
{
	throw new Fatal("HeatElem::Deactivate: Feature not implemented yet");
}
...
inline void HeatElem::_calc_initial_internal_forces()
{
	// Allocate (local/element) internal force vector
