...
int main(int argc, char **argv) try
{
    ...
    Lattice Lat(D2Q9, nu, iVec3_t(nx,ny,1), 1.0, 1.0);
    UserData dat;
    Lat.UserData = &dat;
    ...
    {
        dat.Left .Push(Lat.GetCell(iVec3_t(0   ,i,0)));
        dat.Right.Push(Lat.GetCell(iVec3_t(nx-1,i,0)));
    ...
    for (int j=0;j<ny;j++)
    {
        if (pow(i-obsX,2.0)+pow(j-obsY,2.0)<radius*radius) Lat.GetCell(iVec3_t(i,j,0))->IsSolid = true;
    }
    ...
    {
        Lat.GetCell(iVec3_t(i,0   ,0))->IsSolid = true;
        Lat.GetCell(iVec3_t(i,ny-1,0))->IsSolid = true;
    }
    ...
    //Solving
    Lat.Time = 0.0;
    Lat.Solve(10000.0,20.0,Setup,NULL,"test01");
